...
function sauvegarderDonnees(donnees) {
    ...
    try {
        ...
        // ====== Si quota dépassé, sauvegarder uniquement les colonnes essentielles ======
        if (e.name === 'QuotaExceededError' || e.message.includes('quota')) {
            console.warn('⚠️ Quota localStorage dépassé. Sauvegarde des données essentielles seulement...');
            ...
            try {
                ...
                console.log('💾 Données essentielles sauvegardées (', donneesReduites.length, 'lignes)');
            } catch(e2) {
                // ====== Si ça échoue encore, ne rien sauvegarder ======
                console.warn('⚠️ Impossible de sauvegarder. Les données restent en mémoire uniquement.');
                console.warn('💡 Les données seront perdues si vous rechargez la page.');
            }
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (donneesRestorees) {
        ...
        // ✅ Chargement automatique désactivé - l'utilisateur clique manuellement
        console.log('💡 Utilisez les boutons pour charger les données');
    }
    function chargerFichierDepuisURL(url, nom) {
        console.log('📥 Chargement automatique depuis:', url);
        
        document.getElementById('fileStatus').textContent = '📥 Chargement automatique...';
        document.getElementById('fileStatus').style.color = '#f39c12';
        ...
        fetch(url)
            ...
            .then(function(arrayBuffer) {
                ...
                jsonData.forEach(function(ligne) {
                    ligne['_source_fichier'] = nom + '.xlsx';
                });
                
                traiterDonneesChargees(jsonData, nom + '.xlsx');
                
                console.log('✅ ' + jsonData.length + ' lignes chargées depuis ' + nom);
            })
            .catch(function(error) {
                console.error('❌ Erreur chargement automatique:', error);
                document.getElementById('fileStatus').textContent = '⚠️ Chargement automatique échoué';
                document.getElementById('fileStatus').style.color = '#e74c3c';
            });
    }
});
...
const ZACC = {
    ...
    // ====== Appliquer les filtres (avec les nouveaux noms) ======
    appliquerFiltres: function() {
        ...
        // ====== DEBUG: عرض جميع قيم Sens ======
        var toutesLesValeurs = {};
        this.donnees.forEach(function(d) {
            var sens = String(d['Sens'] || 'VIDE').trim();
            toutesLesValeurs[sens] = (toutesLesValeurs[sens] || 0) + 1;
        });
        console.log('📊 toutes les valeurs de "Sens":', toutesLesValeurs);
        ...
        // ============================================================
        // 2. SÉPARATION DES DIRECTIONS (avec les nouveaux noms)
        // ============================================================
        ...
        console.log('📊 2. Répartition des directions (avec nouveaux noms):');
        console.log('  - Casa-Agadir (croissant):', donneesCroissant.length);
        console.log('  - Agadir-Casa (decroissant):', donneesDecroissant.length);
        console.log('  - autres:', donneesParAnnee.length - donneesCroissant.length - donneesDecroissant.length);
        ...
        html += '<span style="font-size:11px;color:#6b7a8f;">⚠️ Accident grave = accident contenant ≥ 1 Tués ou ≥ 1 BG</span><br>';
    ...
    calculerZACCPourDirection: function(donnees, directionNom) {
        ...
        donnees.forEach(function(d) {
            ...
            if (ZACC.pariteActuelle === 'paire') {
                sectionStart = Math.floor(pk / TRANCHE) * TRANCHE;
            } else {
                sectionStart = Math.floor((pk - 1000) / TRANCHE) * TRANCHE + 1000;
                if (sectionStart < 0) sectionStart = -1000;
            }
            
            var sectionEnd = sectionStart + TRANCHE;
            ...
            // ====== 3. Acc. Graves = accident contenant ≥ 1 Tués ou ≥ 1 BG ======
            var tuesUsager = parseInt(d['Nbr Tués Usagers'] || 0);
            var tuesPietonUsager = parseInt(d['Nbr Tués Piéton Usager'] || 0);
            var tuesPietonVagabond = parseInt(d['Nbr Tués Piéton Vagabond'] || 0);
            var tuesPietonRiverain = parseInt(d['Nbr Tués Piéton Riverain'] || 0);
            var tuesPietonIntervention = parseInt(d['Nbr Tués Piéton Pers. D\'intervention'] || 0);
            var totalTues = tuesUsager + tuesPietonUsager + tuesPietonVagabond + tuesPietonRiverain + tuesPietonIntervention;
            
            var bgUsager = parseInt(d['Nbr BG Usagers'] || 0);
            var bgPietonUsager = parseInt(d['Nbr BG Piéton Usager'] || 0);
            var bgPietonVagabond = parseInt(d['Nbr BG Piéton Vagabond'] || 0);
            var bgPietonRiverain = parseInt(d['Nbr BG Piéton Riverain'] || 0);
            var bgPietonIntervention = parseInt(d['Nbr BG Piéton Pers. D\'intervention'] || 0);
            var totalBG = bgUsager + bgPietonUsager + bgPietonVagabond + bgPietonRiverain + bgPietonIntervention;
            
            if (totalTues > 0 || totalBG > 0) {
